...
```
CI runs on `windows-latest`. Coverage measured with `dotnet-coverage collect`; ≥70% is the target but is not enforced as a hard build gate.
...
## Process
Work follows the QaaS harness pipeline: plan → contract → implement → adversarial evaluation (rubric ≥ 7/10 on Correctness / Completeness / Craft / Robustness). Write tests first (NUnit 4.x, TDD). Use conventional commits (`fix:`, `feat:`, `chore(release):`). Run `csharpier format <changed-files>` before committing.